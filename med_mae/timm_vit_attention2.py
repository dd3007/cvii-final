...
#from util.resnet import resnet26d, resnet50d
#from util.registry import register_model
...
class MaskedAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Introducing Masked Attention: Generate a random mask internally
        # Randomly select half of the positions to mask within the attention scores
        random_mask = torch.bernoulli(torch.full((B, self.num_heads, N, N), 0.5, device=x.device)).bool()
        attn = attn.masked_fill(random_mask, float('-inf'))  # Apply the random mask
